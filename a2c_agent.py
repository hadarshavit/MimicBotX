...
class A2CAgent(Agent):
    ...
    def __init__(self, name,
        ...
        print(self.scripted_actions)
        self.action_queue = []

        # MODEL
        self.policy = torch.load(filename).cuda()
        self.policy.eval()
        self.end_setup = False
        self.scripted_bot = MyScriptedBot(f'scripted{np.random.randint(100, 200)}')
    ...
    def ready(self):
        ...
        # print(device)
        # self.policy.to(f'cuda:{device}')
        pass

    def new_game(self, game, team):
        print(team)
        self.scripted_bot.new_game(game, team)
    ...
    def act(self, game: botbowl.Game):
        ...
        if game.get_procedure().__class__ in self.scripted_actions:
            # print('opp taking scripted')
            return self.scripted_bot.act(game)
        # print('opt taking net')
        self.env.game = game
        # print('Opp', game.get_procedure())
        ...
        _, actions = self.policy.act(
            Variable(spatial_obs.float().cuda()),
            Variable(non_spatial_obs.float().cuda()),
            Variable(action_mask).cuda())
