...
ensure_dir("/local/s3092593/logs/")
ensure_dir("/local/s3092593/models/")
ensure_dir("/local/s3092593/plots/")
exp_id = str(uuid.uuid1())
log_dir = f"/local/s3092593/logs/{env_name}/"
model_dir = f"/local/s3092593/models/{env_name}/"
plot_dir = f"/local/s3092593/plots/{env_name}/"
ensure_dir(log_dir)
...
def main():
    ...
    # MODEL
    ac_agent = torch.load('/data/s3092593/mgai/net_final_good')
    ac_agent.to(memory_format=torch.contiguous_format)
    ac_agent.freeze_except_critic()
    # OPTIMIZER
    optimizer = optim.NAdam(ac_agent.parameters(), learning_rate)
    ...
    while all_steps < num_steps:
        for step in range(steps_per_update):
            with torch.no_grad():
                _, actions = ac_agent.act(
                    ...
                    Variable(memory.non_spatial_obs[step].reshape(-1, non_spatial_obs_space)),
        ...
        Variable(memory.non_spatial_obs[-1].reshape(-1, non_spatial_obs_space), requires_grad=False))[0].data
        ...
        if all_updates % log_interval == 0 and len(episode_rewards) >= num_processes:
            if running_loss / 50 < 0.05 or all_steps > 750_000:
                only_value = False
                print('unfreeze')
                ac_agent.unfreeze()
            running_loss = torch.tensor(0.0, requires_grad=False).cuda()
