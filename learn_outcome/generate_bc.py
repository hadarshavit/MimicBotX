...
class ProcBot(Agent):
    ...
    def act(self, game: Game):
        ...
        if not action.action_type in [ActionType.PLACE_PLAYER, ActionType.END_SETUP]:
            ...
            self.buffer.append((spatial_obs.astype(np.float32), non_spatial_obs.astype(np.float32)))
            # saver.dump()
            # saver.reset()
    ...
    for i in [11]:
        ...
        for as_home in [True]:
            ...
            for j in range(1):
                ...
                # Play a game
                bot_a = MyScriptedBot('s1')
                bot_b = MyScriptedBot('s2')
                ...
                for spatial, non_spatial in bot_a.buffer:
                    buffer.append((spatial, non_spatial, bot_a.result)) 
                for spatial, non_spatial in bot_b.buffer:
                    buffer.append((spatial, non_spatial, bot_b.result)) 
            # print("--------------------------")
